...
class GradCAM_multi2(object):
    ...
    def __init__(self, model, target_layer,std, gradients_flag = False, g_flag = True, g_pp_flag = True, g_sig_flag = True):
        self.model = model
        self.gradients = dict()
        self.activations = dict()
        self.g_flag = g_flag
    ...
    def forward(self, input, class_idx=None, retain_graph=False):
        ...
        if self.g_pp_flag:
            ...
            grad_cam_pp = np.maximum(w * a,0)
            grad_cam_pp = grad_cam_pp * self.std 
            grad_cam_pp = np.sum(grad_cam_pp, axis=-1)
            grad_cam_pp = cv2.resize(grad_cam_pp, (224, 224))

        if self.g_flag:
            grad_cam =a*g_
            grad_cam = grad_cam * self.std 
            grad_cam = np.sum(np.maximum(grad_cam,0),axis=-1)
            grad_cam = cv2.resize(grad_cam,(224,224))
...
class GradCAM_multi_optimistic(object):
    #GradCAM_multi2 for SIG CAM
    ...
    def forward(self, input, class_idx=None, retain_graph=False):
        ...
        channel_size = self.std[class_index].shape[0]
        channel_w  = self.std[class_index].shape[1]
        channel_h  = self.std[class_index].shape[2]
        ...
        if self.g_pp_flag:
            ...
            grad_cam_pp = np.maximum(w * a,0)
            grad_cam_pp = grad_cam_pp * self.std[class_index].reshape(channel_w,channel_h,channel_size)
            grad_cam_pp = np.sum(grad_cam_pp, axis=-1)
            grad_cam_pp = cv2.resize(grad_cam_pp, (224, 224))

        if self.g_flag:
            grad_cam =a*g_
            grad_cam = grad_cam * self.std[class_index].reshape(channel_w,channel_h,channel_size)
            grad_cam = np.sum(np.maximum(grad_cam,0),axis=-1)
            grad_cam = cv2.resize(grad_cam,(224,224))
